...
public class MainActivity extends AppCompatActivity {
    ...
    private static final int NOTIFY_ME_ID = 1;  //고유한 아이디
    ...
    private void notifyMe() {
        ...
        //메시지를 클릭하면 NotifyMessage화면으로 이동하게 처리
        ...
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.O) {
            //채널 사용
            //안드로이드 8.0(API레벨 26)부터 알림 채널을 이용해서 사용자가 알림을 관리할 수 있도록 도와주는 통합된 시스템을 제공

            //채널 생성
            NotificationChannel channel = new NotificationChannel(CHANNEL_ID, CHANNEL_NAME, NotificationManager.IMPORTANCE_DEFAULT);
            
            //채널 설정
            channel.enableLights(true); //라이트 설정
            channel.setLightColor(Color.RED);   //기기가 이 기능을 지원한다면 이 채널에 게시되는 알림에 대한 불빛 색상을 설정
            channel.setVibrationPattern(new long[]{0,1000,500,1000});   //진동 설정
            channel.enableVibration(true);
            channel.setShowBadge(true); //배지로 알림 구현
            channel.setLockscreenVisibility(Notification.VISIBILITY_PUBLIC);    //락화면에서 알림을 어느 정도 보여줄지 설정

            //NotificatioinManager에 채널 등록
