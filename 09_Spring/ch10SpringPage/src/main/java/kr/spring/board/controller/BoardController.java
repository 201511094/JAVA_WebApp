...
public class BoardController {
	...
	//ip주소를 받기 위해 HttpServletRequest을, 회원번호를 받기 위해 HttpSession을 사용
	...
	public ModelAndView process(@RequestParam int board_num) {
		...
		return new ModelAndView("boardView", "board", board);	//객체 생성
	}
	...
	public ModelAndView viewImage(@RequestParam int board_num) {
		...
		mav.addObject("imageFile", board.getUploadfile());	//byte[]타입의 데이터
	...
	public String submitUpdate(@Valid BoardVO boardVO, BindingResult result, HttpServletRequest request, HttpSession session, Model model) {
		...
		model.addAttribute("url", request.getContextPath() + "/board/list.do");	//전체 경로 명시
		
		//tiles설정에 아래 뷰 이름이 없으면 단독으로 JSP를 호출
		return "common/result";	//공통으로 호출하는 View
	}
